...
class SmsWithdrawController {
  public async getAll (req: Request, res: Response) {
    try {
      ...
      const whereWithdrawRequest = Object.assign({},
        transferId == null ? null : { transferId: { [Op.like]: `%${transferId.trim()}%` } },
      );

      if (download) {
        const result = await SmsWithdrawModel.findAll({
          where,
          include: [
            {
        ...
        const wb: any = await ExcelService.listWithdrawSms(result);
        const data = XLSX.write(wb, { bookType: 'xlsx', type: 'buffer' });
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        return res.end(data);
      }
      const { rows, count } = await SmsWithdrawModel.findAndCountAll({
        paranoid: false,
        where,
        include: [
          {
      ...
      const sumSuccess: any = await SmsWithdrawModel.findAll({
        paranoid: false,
      ...
      const sumPendding: any = await SmsWithdrawModel.findAll({
        paranoid: false,
  ...
  public async update (req: Request, res: Response) {
    try {
      const smsLog = await SmsWithdrawModel.findByPk(req.params.id);
      if (!smsLog) {
...
export default new SmsWithdrawController();
